...
class DecisionRecord:
    ...
    # NO day_rank / cohorts / agreement FIELDS HERE, DELIBERATELY.
    #
    # A day-level rank is not knowable when this record is written: run_slate
    # writes decisions game-by-game, and the rank of a selection depends on
    # every other game on the slate. More importantly, the ranked slip has to
    # be FROZEN rather than re-derived -- re-deriving it later under a newer
    # ranker is precisely the re-ranking of graded history that
    # docs/PRODUCT_DOCTRINE.md amendment 8 forbids -- so it must be persisted
    # somewhere, and the moment it is persisted, that artifact is the
    # authority on rank.
    #
    # Duplicating rank onto the decision would create a second authority on
    # the same fact, which is the failure mode this module exists to prevent
    # (the sign has no field to live in; the objective has no field for
    # money). The published slip lives in its own append-only ledger and owns
    # rank, cohort and agreement outright. `COHORTS`/`COHORT_IMPLIES` above
    # are the shared contract it writes against.
    prev_hash: str = ""
    row_hash: str = ""
