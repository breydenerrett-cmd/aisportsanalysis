...
# MOVED TO src/pipeline/enrichment.py ON 2026-09-10, and re-exported here so
# every existing caller and test in this module keeps working unchanged.
#
# It had to move because src/cli.py's `card publish` needs the SAME inputs
# and src/ may never import from api/ (tests/test_api_boundary.py). That is
# not a style point: the CLI was building its slate WITHOUT these inputs, so
# the card it froze into the public ledger was a different card from the one
# the page served for the same date -- four picks against three, because a
# missing arsenal changed one game's starter features and flipped the
# model's agreement. Receipts that disagree with the page they are receipts
# for are worse than no receipts.
_latest_weather_by_pk = enrichment.latest_weather_by_pk
_enrichment_inputs = enrichment.enrichment_inputs
