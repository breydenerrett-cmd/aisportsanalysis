...
## 2026-09-05 22:5xZ — CAPTURE INCIDENT: in-session runs died mid-sleep 17:15–22:15Z
- Every hourly run from 17:15Z on wrote only its FIRST dense capture (rows at :16 each hour, 493–713/hr) then died silently during the 15-min sleep: no 15/30/45-min grid points, no close pass, no F5 closes (store unchanged since 02:01Z), no prop listing, no extras, no commit for six hours. Health helper read HEALTHY_IDLE off artifact age; the heartbeat leg (379m) was the true signal and it was in the status line but not acted on for two cycles. Lesson: heartbeat age > 2h while games are in window must be treated as FAILED even when artifacts are fresh.
- Cause: container reclaimed between triggers (uptime 3 min at 22:39Z). `nohup ... &` inside a subshell is not harness-tracked and does not survive reclamation. Runs 14–16Z survived only because no game was in window and they finished in ~2 min. This is the exact failure L25/W2 externalization exists to remove; still BLOCKED_HUMAN on the ODDS_API_KEY Actions secret.
- Actions: manual commit a860255 of the on-disk first-capture rows (nothing lost from disk; disk persisted across the restart). Runner relaunched 22:50Z as a harness-tracked background task (run_in_background), which is the launch path the earlier 47-min runs that completed used. Verification at the 23:15Z trigger: do NOT start a second run if the tracked one is alive; check `capture_2250.log` reaches "== committed ==" at ~23:37Z.
- One self-inflicted false negative on relaunch: a `pgrep -f 'src.cli dense'` guard matched its own command line (the exact naive-pgrep failure Brey named) and skipped the run; relaunched without it. Data lost and unrecoverable: grid points 17:31–22:46Z, F5 closes for the 20:11Z and ~22:xxZ games, prop listing rows for those hours.
- 23:4xZ: recovery verified. Tracked run 22:50→23:37Z completed end to end: 4 captures / 351 observations, +54 F5 closes (store 620), prop listing + extras ran, committed d41bd59, no ESCALATE. Container stayed up through the 23:15Z trigger. Standing rule for the rest of the weekend: launch `forward_capture.sh` ONLY as a harness-tracked background task, and skip a trigger firing when a tracked run is still alive (check via `ps` on `forward_capture.sh`, never `pgrep -f` from a shell whose own command line contains the pattern).

## 2026-09-06 10:5xZ — daily loop: 2 ESCALATEs, root-caused and fixed (76140d5, data 18f9344)
- Slate 09-06 refused: Statcast pitch store coverage ended 09-02 (4d > 3d guard). Nothing on the daily cadence ran `statcast --catchup`; last run was a manual one on 09-03. Ran it now (free, 11,663 rows for 09-03..09-05), slate re-ran exit 0 (90 wagers, all with game_pk).
- Settle 09-05 refused: all 99 wagers written with game_pk=None because the event->game_pk map had no rows for 09-05 — nothing extended it either. Wager rows are hash-chained and cannot be edited, so `run_settle` now resolves a missing pk from the map at settle time (identity join, never overrides a slate-resolved pk, still refuses anything unresolved). `run_slate` now extends the map for its own date before reading it. Both prerequisites added to `daily_loop.sh` before `engine slate`, each with exit capture + ESCALATE. Settle 09-05 re-ran: 99 settlements, bankroll +0.46%, scorecard REFUSE (bankroll-only — correct). eod 09-05 regenerated.
- Regression tests: settle map-resolution (+ no-override), slate refresh-before-read (+ failure degrades, injected map skips), daily-loop wiring order. Engine 174 + gamekey 23 tests green.
- Pattern across Fri/Sat/Sun: three separate guard refusals were all "a store the guard reads had no refresher on the cadence" (L1 projection, pitch store, gamekey map). Every guard's input now has a refresh at the point of read or a loop step.
