...
|---|---|---|---|
| L1 | **Family V3 — information timing / market microstructure**: when genuinely new information enters, how fast do books react, which move first, and does an observable stale-price window exist? (docs/RESEARCH_V3_TIMING.md) | **FROZEN 2026-08-31, forward-only, accumulating. No event-response result exists anywhere yet** — nothing is readable until a class reaches its 30-event floor. Family denominator **4** admitted classes: `lineup_posted`, `starter_scratch`, `hitter_scratch`, `il_roster_move`, all grade B (bracketed between our own polls). Primary hypothesis per class: median time to 50%-of-books reaction exceeds the capture-spacing floor. Gates: ≥6 books quoting pre-event, minimum meaningful move 0.010 de-vigged, BH-FDR q=0.10 across the family. Capture: hourly baseline, 15-minute brackets in dense windows. | A class hitting 30 admitted events produces its first read. **V3 claims no edge by construction**: measurable latency is NECESSARY, never sufficient — executability, limits and price-vs-fair stay separate questions, and any "bet the stale price" claim would be its own registered family with the full funnel. A null is a publishable result, with the resolution and cost that would be needed. |
