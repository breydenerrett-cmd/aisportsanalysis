...
|---|---|---|---|
| L1 | **Family V3 — information timing / market microstructure**: when genuinely new information enters, how fast do books react, which move first, and does an observable stale-price window exist? (docs/RESEARCH_V3_TIMING.md) | **FROZEN 2026-08-31, forward-only, accumulating.** Family denominator **4** admitted classes: `lineup_posted`, `starter_scratch`, `hitter_scratch`, `transaction_first_seen` (named `il_roster_move` in the freeze record), all grade B. **First class read, 2026-09-02 (ADDENDUM in docs/RESEARCH_V3_TIMING.md):** `transaction_first_seen` reached 56 measurable events (past its 30-event floor) and its primary test ran — per-event paired diff (reaction to 50%-of-books vs. that event's own capture-spacing floor), Kaplan-Meier for the 69.6% of events censored at first pitch, clustered bootstrap by game. Point estimate S(0)=1.000 (every observed and lower-bounded diff exceeds its floor), 95% CI [1.000, 1.000], one-sided p=0.000, split-half replicated — but the CI is a boundary artifact of a 56-event/20-cluster sample (the bootstrap cannot draw below its own observed minimum), so read it as "no counterexample yet," not certainty. No promotion: BH-FDR q=0.10 needs all 4 classes' p-values (early deaths at 1.0), and `lineup_posted` is one event from its own floor. The other three classes remain unread. | A class hitting 30 admitted events produces its first read. **V3 claims no edge by construction**: measurable latency is NECESSARY, never sufficient — executability, limits and price-vs-fair stay separate questions, and any "bet the stale price" claim would be its own registered family with the full funnel. A null is a publishable result, with the resolution and cost that would be needed. |
