...
| research-readiness-battery | 2026-09-15 | forward-test system(s) now have 30+ graded selections | orchestrator, per Stage 16 R16-06 | 2026-09-15 | fix in progress | `scripts/research_readiness.py` correctly reports that 14 forward-test systems have crossed the falsification battery's `MIN_N` (30 graded selections, `src/research/battery.py`). Wiring `src/research/battery.run` into `src/engine/settle_slate.py`'s `build_scorecard` call is in progress today; until that lands, every scorecard still reads `NOT_RUN`, which the audit is right to keep flagging. |
