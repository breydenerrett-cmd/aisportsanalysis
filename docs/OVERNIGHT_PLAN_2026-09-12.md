...
## Tier 3 — make tonight's new data usable tomorrow

Two pipelines changed today and both produce their first real output
overnight. Build the readers now so nothing waits on me in the morning.

- **Verify the prop capture actually moved.** `CAPTURE_LEAD_MINUTES = 120`
  shipped today; before it, all 9,672 batter-prop quotes landed 04:00-09:10
  UTC. **Done when** the store holds a quote taken within two hours of a
  first pitch — the first one this product has ever had.
