...
|---|---|---|---|
| W-1 | Tue 9/16, first | **Make the daily loop green on its own schedule.** It settles picks, grades the record and writes the day's evidence; it has been failing on schedule since at least 9/13 and succeeding only on manual dispatch. Diagnose from the scheduled runs' logs, fix the cause, and prove it on a scheduled run, not a dispatch | Two consecutive scheduled runs green, with the escalation ledger showing only acknowledged lines | RUNNING | 2026-09-16 05:00Z: diagnosed. The last scheduled failure (34982129696, 09-15 14:30Z) escalated on exactly the two acknowledged lines and predates the ledger check, which landed at 502368c1 and was synced to the default branch at b0d73d8f. Verified locally: the two standing lines give exit 0 and a third, unknown line gives exit 1; the default-branch workflow's verdict step is `python3 scripts/escalations.py --check`. **Trap for the next run: a plain `git fetch origin <branch>` does not move the local `origin/<branch>` pointer, so reading the default-branch workflow through that ref shows a stale file. Read it from the sha `git ls-remote` reports, or fetch with an explicit refspec.** Proof outstanding: the 10:00Z scheduled run, then one more  **Diagnosis corrected 2026-09-15 22:10Z:** the 14:30Z scheduled failure (34982129696) did not fail on escalations at all -- its exit 1 came from `src.cli budget`, where the tennis_h2h credit probe passed `provider.fetch_odds()`'s list of events into `_payload_shape`, which assumed the single-event dict (`AttributeError: 'list' object has no attribute 'get'`, budget.py:711). Fixed at d540d481 (the `is_multi_event` branch), committed 21:12Z -- AFTER that run, which is why the run predating it still shows the crash. The fix is an ancestor of the remote branch head the scheduled job checks out (`ref: claude/sports-betting-analysis-review-g1o0co`), and the 20:52Z dispatch on that branch was green in 12m10s. The settlement step now calls `python3 -m src.appstate.live_ledger settle` (ebf04b4e, pushed); run against the real store it prints `graded=0 voids=0 unsettled=0 dates_checked=1` and is `|| true` guarded, so it cannot fail the job. Proof still outstanding: the 2026-09-16 10:00Z scheduled run, then one more. |
...
| W-3 | Tue 9/16 | **Card V2 build starts** on the parts that need no owner answer: T1 pre-build checks, T2 the pure rule module, T3 the V2 ledger, T4 closing-line measurement, T2v the variant runner, T3v per-variant ledgers. Registration (T0) waits on questions 12 to 14 | Each task's named tests green; nothing registered; no customer surface changed  T1, T2, T3 and T4 DONE 2026-09-16 | T2 at 31cdf378: `src/analysis/best_bets_card.py`, the registered rule as a pure module -- RuleParams with the price band, separate MAIN and PLUS_MONEY floors, the plus-money sub-cap and a ceiling counting fills; price_class, marked_down, required_edge, value_need, score, failed_gates, is_fill_eligible, rank_key, select(), and the card's copy. No SHADOW_D (registration section 10 deregisters it) and a test pins that. Arithmetic hand-checked against the registration's own 4.2 table. T3/T4 at 5a95931f: the V2 ledger added additively to `card_ledger.py` (six V1 ledger modules still green), price_class frozen at write time, raw and marked-down numbers and score on every entry, and the two gates that select() alone cannot enforce -- a locked plus-money pick from an earlier run plus a fresh one can exceed the sub-cap when neither run's own pool did -- applied in registered order G11, G14, G12, with locks never demoted. Both invariants proven by unwiring the fix and publishing for real: 12 picks against a ceiling of 10, 5 plus-money picks against a sub-cap of 3. T4 is `src/report/card_clv.py`, stamping price_class on every measurement with an explicit absence vocabulary so an unmeasurable row is never silently dropped from a denominator. 166 tests green across the V2 modules plus the V1 regression set. STILL OPEN: T2v and T3v (the variant runner and per-variant ledgers), and T0 registration, which waits on owner questions 12 to 14. |
...
| W-5 | Wed 9/17 | **Live pipeline able to record** (R16-L2 to R16-L7): pre-game context and keys, in-play credit accounting from the response headers, the push path, rule-gated capture with freshness, ledger rows, settlement from authoritative finals | A real MLB window writes candidate rows with their in-play prices, inside the 300-credit cap, verified by a separate checker  CODE COMPLETE, ACCEPTANCE PENDING | 2026-09-16 07:15Z. All six build items are in: L2 pre-game context and keys (0519d829, `live_pregame_check` prints 15 of 15 usable games for 2026-09-15 with favourite, probability, 11 books and a starter); L3/L4 in-play credit billed per call from `response['usage']['last']` into its own `data/live/credit_log_live.jsonl`, so the window and the capture chain stop fighting over one file (a9c10ed2); L5 capture gated on a registered rule trigger with a T0/+45s/+90s retry ladder, a non-gating +5min follow-up and a 3-book freshness median, plus D8, which had been reading `status.abstractGameState` -- a key `parse_game()` never returns -- and counting every past start as live, so a final or cancelled game held the window open and kept spending (921d0fd8); L6/L7 ledger rows and settlement from authoritative finals with a 7-day VOID rule, called by the daily loop (ebf04b4e). `python -m unittest` over the six live modules: 185 OK. **The row is NOT crossed off**, because its acceptance is not a test count -- it is a real MLB window writing candidate rows at real in-play prices inside the 300-credit cap, checked by someone other than the builder. No MLB game is in play at this hour; the run is queued for today's slate. KNOWN SEAM carried forward: window-gap rows write to `data/live/<sport>/window_gaps.jsonl`, not the hash-chained ledger, because `live_ledger.py` was out of scope for the builder that produced them; the wiring point is documented in `record_window_gaps`. |
| W-6 | **Thu 9/17, kickoff 5:15pm PT** | **NFL forward testing begins**: Thursday card published and locked before kickoff, live window during the game under the cap, settled Friday morning. NFL stays internal and coming-soon on the site | Card row frozen before kickoff; live rows or an honest "no trigger fired"; settlement Friday; nothing published to customers  UNBLOCKED 2026-09-16, commit 9a45be1a -- and it would have failed silently | Checking a day early, the Thursday dry run produced nothing. Three faults, none of them the model: (1) `prices.boards_by_matchup()` resolved team names through the MLB-only abbreviation table for every sport, so every NFL row was dropped and the function returned an empty board dict on EVERY date -- this path had never produced an NFL card and could not have; (2) the empty card then printed "No NFL game cleared the bar for this date", which reads as the model declining a bet, so a dead path wore the wording of an honest null -- the failure mode this project is most exposed to, because a null IS the expected answer here and nobody looks; (3) `--dry-run` was never read in the NFL branch, so every dry run published a real row to `evidence/cards_nfl_v1.jsonl`, and the branch read a key the success shape does not carry, printing "no card: None" on a genuine pick. Two rows were written accidentally while diagnosing; both removed, neither ever in git history (verified: no `evidence/cards_nfl*` file, `git log --all` on that path empty). The tests missed all three because they covered the functions and not the CLI path a person runs; `tests/test_cli_nfl_card_publish.py` now covers it, two of its three tests proven failing against the pre-fix CLI. `official_date`'s Eastern rule was checked across all 4,698 captured NFL rows and is correct for every kickoff slot. Thursday now builds: Lions @ Bills, 11 books, consensus 66.5% Bills at -218 -- note that is a -218 favourite, exactly the price the owner ruled out on 2026-09-15, so V1 and V2 will disagree about it. 132 tests green.  **READINESS WALKED 2026-09-16, ahead of kickoff.** After the card fix, the rest of the Thursday path was checked step by step rather than assumed, because the card had just proved that a step can look fine and have never run. Findings: the per-pick freeze is the SAME code MLB already exercises (`card_ledger.publish(sport="nfl")` -> `lock_lead_for("nfl")`, 4.0h), not a separate NFL branch; settlement is already wired into `daily_loop.sh` and no-ops safely when nothing was published; `capture_slot.sh` already runs `card publish --sport nfl` and the NFL dispatch check every slot. The live window is NOT in the card's old never-could-have-worked state -- `live_window.py` is genuinely sport-parameterised with its own NFL tests, one registered live rule (`nfl_favorite_trails_halftime`), and `should_dispatch("nfl")` correctly returned `HOLD outside NFL broadcast window (Thu 19-23, Sun 12-23, Mon 19-23 ET)` when asked today. BUT it has never fired in anger: `data/live/nfl/` does not exist on disk and has no git history. That is the single biggest Thursday risk, and it is the unobserved kind, not the broken kind -- somebody should watch the 7-11pm ET window rather than assume it, which is exactly the assumption that hid the card bug. **V1 vs V2 on tomorrow's game, run dry and registering nothing:** V1 publishes `#1 [STRONG] Take Buffalo Bills to win at -218` ranked on market confidence alone (66.5%, 11 books, `model_probability: null` -- our own numbers have too few games to weigh in). V2's `select()` returns `[]`: `price_class(-218)` is None, outside both MAIN -160..-100 and PLUS_MONEY +100..+250, refused at G4_BAND on price alone. So the live rule will publish precisely the price the owner ruled out on 2026-09-15, and the rule that would refuse it is not registered (blocked on owner questions 12-14). Registration is not something to shortcut, so this is recorded, not resolved. Also removed: `card publish --force`, a flag whose help text promised to override a pre-first-pitch veto that stopped existing when the freeze became a preview label plus per-pick lock -- no code read it for either sport and no caller passed it. |
...
| W-15 | Wed 9/16 | **Statcast pitch store for 2023 and 2024**, the gap W-4 exposed: the matchup matrix's starter-mechanics nulls are gated by `data/historical/statcast`, which is absent on this machine and holds nothing for those seasons. Fetch it the repo's own way (find the builder, free source, point in time), then rebuild the two matrices and publish the before-and-after null rate per field | Matrix starter-mechanics null rates fall and are published per field; no odds credits spent; rebuild is deterministic and does not touch the sealed window | OPEN |  HALF DONE, HALF BLOCKED 2026-09-16 | The Statcast store is built and is the real deliverable: the repo's own free keyless Baseball Savant fetcher (`src/providers/statcast_pitches.py`, already used by `src.cli statcast --catchup`) ran 47 of 47 windows for each season -- 2023: 720,984 rows; 2024: 711,456 rows; 0 failed, 0 errored; 94 window files, 24MB. Sanity: ~297 and ~293 pitches per game against 2,430/2,429 games, inside the normal 270-310 band, so nothing was silently truncated. Deterministic by construction: a second `build()` for both seasons returned `windows=0 skipped=47`, every window already manifested, nothing refetched. Single sequential writer, so the one-writer-per-file rule was never at risk. Zero odds credits; sealed window untouched. **BLOCKED on the second half, and on a data claim that does not hold.** Rebuilding the matrices needs a posted lineup per row, and `data/historical/lineups.jsonl` on this machine has 119 rows, recent dates only -- while W-4's own note claims 2,440 and 2,434 lineups were fetched for 2023 and 2024. The committed matrices contain real data, so that store existed when they were built and is now gone or never persisted here. Running `matrix.build(force=True)` against the thin store wiped both matrices to `{"empty": true}` markers; the builder caught it immediately and restored both from HEAD, verified by `git diff --numstat` as no textual change (the only difference is LF/CRLF), and nothing was committed. So the after-rates cannot be measured yet. BEFORE rates, from the restored files, for the record: 2023 (2,430 rows) starter_platoon_gap 51-52%, starter_velocity_gap 33%, starter_groundball_share 29-30% null; 2024 (2,429 rows) 16%, 14%, 8%. Finish under W-18. |
| W-16 | Tue 9/16 | **The private in-play watcher** (`docs/SNIPE_SYSTEM.md`, committed 6b11a750): owner wants it watching tennis and NBA, paper only, on his always-on Windows desktop, alerting by push with a session page. Build order is that document's section 7, and its first two items are deliberately cheap kill switches: fix the live plumbing (W-5), then measure for a week whether our feed sees a state change before the books move | One real match watched end to end, one paper ticket written or an honest no-trigger, the ledger chain valid, one alert delivered, and nothing claimed about whether the bet was good | BLOCKED_ON W-5 | 2026-09-16: design committed; three owner decisions open in its section 0 (API-Tennis trial, price floor -150 versus -200, conviction label versus a shadow staked account)  Owner answered two of three, 2026-09-16: **price floor** -- allow to -200 ONLY where the model's own post-markdown number is 0.75 or better, which section 4 shows is the only region where -200 clears the value test; anything worse than -150 outside that region stays `OUTSIDE_BAND`. **conviction** -- a shadow staked account IS run alongside, sized by conviction, off the same rows, and it must never feed the gate: the evidence ledger stays flat 1 unit. **API-Tennis** -- deferred until the full-day sample finishes (W-17), which is the last moment it can be deferred to, since the key expires 2026-09-18. Both answered decisions must be written and dated into the pre-registration that relies on them BEFORE the first observation, per this document's own section 0 rule. |
| W-17 | Tue 9/16 | **API-Tennis: decide before the trial key expires on 2026-09-18.** First pass measured (`docs/API_TENNIS_TRIAL_RESULTS.md`, commit a9b7477b): set betting quoted on 8 of 19 live matches against an 80 per cent threshold, the suspension flag wrong on 104 of 638 moments and always in the direction of showing a price while suspended, price latency median 23.6 seconds against a 10 second threshold, point log clean over 30 sets. Three checks need a longer run than one pass, so run the poller across a full day, then record a dated decision | `docs/API_TENNIS_TRIAL_RESULTS.md` carries a full-day sample for checks 7, 9 and 10 and a dated buy or skip; if skip, `docs/SNIPE_SYSTEM.md`'s tennis rule is narrowed to match winner on the face of the rule | OPEN |  **DONE 2026-09-17 -- SKIP, nothing paid.** Full day sampled at production rates: 367 polls, 25,037 calls, 8,214 matches seen, 5,102 latency triggers. Four of five checks fail and the samples are far too large for noise. Check 9 freshness is the disqualifier: median **54.0s** against a 10s bar, worst 89.7s against 30s. The owner's whole in-play premise is seeing a price move before the book does; at 54 seconds we are the last to know. Check 8 is independently disqualifying and worse in kind -- **104 moments priced while the market was suspended** -- because a phantom price enters a paper ledger as a real bet and poisons the evidence. Check 7 second-set markets quoted on only 59.9% of matches against 80%. The only PASS is check 10, volume: 25,037 calls against the Business plan's published 200,000/day, about 12.5% -- ample quota to poll a feed too slow to bet into. The websocket objection was raised and closed: the vendor documents one socket carrying scores and point-by-point only, and a probe of 15 messages across 24 matches found zero odds, price or suspension fields, so there is no faster transport with prices at any tier. Consequence, recorded on the face of the rule rather than in a footnote (`docs/SNIPE_SYSTEM.md` section 6.1): the tennis rule narrows from set_winner/set_2 to **match_winner/match**, which is a SUBSTITUTION -- it pays less, carries the rest of the match's risk, and is not the mispricing the owner is hunting. Revisit only for a feed whose in-play prices arrive inside ten seconds and which never quotes a suspended market; not because this one gets cheaper. Key expires 2026-09-18, not renewed. **Method note worth keeping:** a single 11.9s reading overnight looked like the feed being fast off peak and was reported as such; the full-day median came in at 54.0s, WORSE than the morning's 37.8s. One reading is not a trend, and the full-day sample existed precisely to stop that. |
| W-18 | Wed 9/17 | **Restore the 2023-24 lineup store, then finish W-15's measurement.** `data/historical/lineups.jsonl` holds 119 rows on this machine against W-4's claim of 2,440 and 2,434 for those seasons, and without it `matrix.build(force=True)` wipes the matrices instead of improving them. Rebuild via `src/pipeline/lineup_store.build()` from the same free StatsAPI source W-4 used (~4,800 game-dates), THEN rerun the matrix build and publish the per-field null rates before and after against the numbers recorded in W-15. Also establish why the store is absent while its downstream output was committed -- if a build product can be committed while its input is lost, other 'already fetched' claims in this document may be equally hollow and should be spot-checked | Lineup coverage restored for both seasons; matrices rebuilt twice with byte-identical output; per-field null rates published before and after; a dated note on how the input went missing, and a spot-check of at least two other completed data claims | OPEN | |
...
| # | Item | Acceptance |
|---|---|---|
| H1 | **The never-ran audit.** For every production code path that can emit a customer-visible or evidence-bearing artifact, determine whether it has EVER produced one. The NFL card had not, and nothing said so. Walk the stores and the git history, not the tests. Produce a register: path, artifact it should produce, first and last time it actually did, verdict LIVE / NEVER-RAN / DORMANT | `docs/NEVER_RAN_REGISTER.md` lists every such path with evidence; each NEVER-RAN entry gets a roadmap row or a documented reason it is expected to be empty  DONE 623f2b03 | 45 paths audited: 24 LIVE, 9 DORMANT, 12 NEVER-RAN of which 7 are expected and cited. Five ALARMING -- the live window's event store, candidate ledger and gap log (one mechanism: capture_slot.sh dispatches every slot and swallows gh failures behind || true), the NFL card, and an F5 results file two modules load that has never existed. `docs/NEVER_RAN_REGISTER.md`. |
| H2 | **The join-integrity audit.** The NFL bug was a join that silently dropped every row, and a copy sweep flagged the MLB card's `_game_identity` vs `gamepayload.game_id()` as possibly the same shape. Find every join between two stores or two id constructions; for each, assert non-empty on real data and fail loudly when a join yields zero | Each join has a test that starves it and asserts the failure is DISTINGUISHABLE from an honest empty; the MLB card id-join question is answered with real data  DONE ab89aa77 | The MLB card's ids agreed by LUCK, not by construction: card._game_identity hand-rolled the same string gamepayload.game_id() builds, and they matched on all 15 games only because every game that day carried game_number 1. A game missing both game_number and game_pk would have diverged silently and read as "the books have not posted prices yet". Now one function on both sides. `src/joins.py` makes a zero-match join from non-empty inputs escalate while an honest empty stays quiet -- verified live: zero moneyline rows exist for 09-17, the guard stayed silent, the sentence was true. |
| H3 | **Empty-output honesty sweep.** Every "nothing to show" message must distinguish *nothing to evaluate* from *evaluated and declined*. Fixed for the NFL card; do the rest | Every empty branch emits one of two distinct reasons; a test per branch  DONE e9ec3f96 | Most surfaces were already honest. One real collapse, in the price board: the same sentence covered "nothing captured" and "every matchup too thin to price". Split. The dashboard synthesis headline has the same collapse and was NOT softened to cover both -- it needs a contract change at synthesize(), so it became H6. |
| H4 | **Store contracts.** The shrink guard covers three files by name. Generalise: every append-only store declares itself append-only, and the guard reads that declaration rather than a hand-kept list | A new store is protected by declaring it, not by remembering to edit a script; test proves an undeclared store is caught by a lint  DONE 35b3801e | A store is protected because it declares itself in `scripts/append_only_stores.txt`, not because someone remembered to edit two scripts. Proved by declaring a store the repo has never heard of, clobbering it 900 rows to 7, and watching the guard restore all 900. Nothing new was declared: every store an automated job stages was walked commit by commit and none has ever shrunk, and that negative is recorded so nobody walks it again blind. The lint states what it cannot detect. |
| H5 | **Monitor-target correctness.** health.py was reading a different file than it named. Audit every health/alarm check: does it read the artifact it claims to describe? | Each check names its artifact in code and a test asserts the path it reads matches the thing it reports on  DONE 671307c2 | Four monitors that could not see what they claimed. The whole-suite forward-store guard gated on a Linux-only /proc probe, so it had NEVER run here while announcing "forward_capture.sh is running" as a false reason -- it now reports OK across 10 stores. publication_audit compared the card's own first_pitch_utc to the clock, auditing the card against the card, beside a sibling whose docstring says "a check that calls the code under test is an echo, not an audit". Settlement dropped orphan rows. And the lineup check survived its own fix at one remove, counting only today's rows. Two more flagged unfixed as H8 and H9 rather than repaired blind. |
| H6 | **The synthesis headline cannot say which nothing it is** (found by H3, 2026-09-16). `dashboard._synthesis_section` prints `NO_EDGE_HEADLINE` -- "Interesting matchup, but no demonstrated betting edge" -- both when synthesis had NO findings for a game at all and when it ran, found candidates and suppressed every one (no sample, below floor, duplicate, outside top five). The suppressed list rendered underneath does distinguish them for a reader who expands it, so the fact is recoverable rather than hidden, which is why this is a row and not an incident. H3 correctly refused to soften the sentence to cover both cases -- that is the papering-over the whole H series exists to prevent. The real fix is at the analysis boundary: `synthesis.synthesize()` must distinguish "ran with zero input findings" from "ran and suppressed everything" in what it RETURNS, which is a contract change, not a wording change | `synthesize()` returns the two cases distinguishably; the headline names which one it is; a test pins that they read differently, failing against today's code | OPEN | |
| H7 | **The suite cannot tell me whether I broke something.** Full-run failure counts observed on 2026-09-16 alone: 40+2, 38+1, 42+1, 43+3, 18+1 -- the same tree, the same day. Some of the spread is the smaller `test_fast.sh` versus the parallel runner, but not all of it, and the parallel runner alone moved between 18 and 43. Causes seen: tests that read stores a live capture job is mutating mid-run (the runner already prints `SKIPPED forward-store fingerprint check: scripts/forward_capture.sh is running`), plus genuinely load-sensitive assertions (peak RSS reading 0.0, a capture-freshness window). Consequence: EVERY agent today had to eyeball the failure list and rule it unrelated by hand -- which is the human filtering that misses the one that matters, and it is how three stale `live_window --settle` assertions sat red for hours while being read as known noise. The repo already has the right pattern for this in `scripts/escalations.py`: classify KNOWN versus NEW and exit non-zero only on NEW. Apply it to the test suite -- a checked-in baseline of tests known to fail on this platform, with the REASON for each, and a runner that reports NEW failures separately and loudly. Tests that read a live-mutating store get an injected seam or a skip that states why, never a silent pass | A run prints `NEW: <test>` or nothing; the baseline file names every known failure with its cause and platform; the same tree run twice produces the same verdict; no agent has to judge a failure list by eye again | OPEN | |
| H8 | **The capture health check cannot run on this machine, and its freshness signal is the false-outage shape** (flagged by H5, 2026-09-16). `src/capture/health.py`, invoked by `scripts/capture_health.sh`, imports `fcntl` at module level, so on Windows it fails at import -- it is already a known suite failure and was being read as environment noise rather than as a monitor that does not exist here. Worse than the import: its freshness signal is local file mtime against wall clock, plus a git-log heartbeat that returns None silently outside a checkout. That is exactly the shape that manufactured a phantom outage tonight -- local mtimes on a store that CI owns and commits look hours stale while production is healthy. H5 declined to fix it blind because verifying it needs a Linux box, which is the right call: a monitor repaired without being run is a monitor you trust more and should trust less | Runs on both platforms or says plainly which it cannot see; freshness derives from the committed artifact rather than local mtime; verified on Linux before it is believed | OPEN | |
| H9 | **`apphealth.FORWARD_STORES` lists 6 of the 9 stores the repo calls forward evidence** while its own docstring says "each forward store" (flagged by H5, 2026-09-16). A list that silently covers two thirds of what it claims is the same defect class as the lineup check reading the watch log: the report is true about what it read and false about what it says it read. Related to H4 -- the answer is probably that the forward stores declare themselves the way append-only stores now do, rather than being enumerated by hand in a second place | One declaration of what a forward store is; apphealth reads it; a test fails when a store exists that no declaration covers | OPEN | |

### B — Build (finish what is started)

| # | Item | Acceptance |
|---|---|---|
| B1 | Card V2 T2v and T3v — the variant runner and per-variant ledgers (T0 registration still blocked on owner questions 12-14) | Named tests green; nothing registered  DONE c6feea37 | Four arms over one shared pool, with pool_hash over the POOL rather than any arm's output -- that is what proves they saw the same board. Deliberately no per-arm record function, pinned by a test, because with four arms on one pool a number you can watch is a number you steer by. T0a landed too (c59300fd): the 2025 fit is frozen at sha256 674790B5..., deterministic across two runs, and the sealed-window guard kills the process rather than filtering a row. |
...
| B3 | Tennis rules narrowed to match-winner if W-17 decides skip | The substitution is stated on the face of the rule, not in a footnote  DONE 3ba57eb8 | W-17 decided SKIP on a full day's evidence, so the tennis rule narrowed from set_winner/set_2 to match_winner/match, written into the rule body and labelled a SUBSTITUTION -- it pays less, carries the rest of the match's risk, and is not the mispricing being hunted. |
...
| # | Item | Acceptance |
|---|---|---|
| D1 | **Gameday card** (W-13 group 2) — the customer's nightly touchpoint, and the surface the copy sweep says to fix before anything visual sits on it | Acceptance per DESIGN_BUILD_PLAN.json; verified at three widths  DONE d248dbe3 | The card leads with what a reader decides on and everything else sits behind View breakdown. STRONG/LEAN/SPLIT removed on purpose: those labels were driven by MARKET confidence, so a short-priced favourite wore the same badge as real value -- the owner's own 2026-09-15 complaint. Also fixed: the experimental notice was being wiped by the empty and error branches. VERIFIED no horizontal overflow at 375/768/desktop by measurement; NOT visually verified behind the invite-token sign-in, and the group's own tests_owned file test_web_gameday_card.py does not exist. |
