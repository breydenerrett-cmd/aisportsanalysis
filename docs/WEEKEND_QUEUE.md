...
| W10 | P1 | Factory scale prep: canonical wager store, strategy→wager refs, unique-wager counts, correlation clustering, retire/mutate/replace framework | – | Sonnet | RUNNING (design + first slice) | design + tests | src/evolab/, docs/ | – | – |
...
| W15 | P1 | Totals B6 population audit: counts-only effect of >=3-book floor × half-point restriction; closing-line gap distribution + staleness bound; bucket-occupancy tables for population-shift test | W9 | Sonnet + bash | RUNNING | docs/TOTALS_POPULATION_AUDIT.md, script, no outcome read | scripts/totals_population_audit.py, docs/TOTALS_POPULATION_AUDIT.md | – | – |
| W16 | P1 | Totals standalone evaluation path mirroring f5_eval (validation items 1-13, mechanised gates, frozen record, bounded spec hash) | W15 | Sonnet | DEFERRED | tests green; dry_run on real data | src/research/totals_eval.py, tests/ | – | – |
| W17 | P1 | Totals pre-registration draft (family of two per re-review B2) → Opus methodology review → adversarial review → register | W15, W16 | Sonnet + Opus | DEFERRED | VERDICT lines | docs/PREREG_TOTALS_FAMILIES.md | – | – |
