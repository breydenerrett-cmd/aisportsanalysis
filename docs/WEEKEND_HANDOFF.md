...
- Capture: healthy Friday evening through Saturday 16Z. **Saturday 17:15–22:15Z incident**: six consecutive hourly runs wrote only their first dense capture then died mid-sleep when the container was reclaimed between triggers (uptime 3 min at 22:39Z). Lost, unrecoverable: the 15/30/45-min grid points for those hours, F5 closes for the 20:11Z and ~22Z games, prop rows. First-capture rows were on disk and committed manually (a860255). Root cause is the one W2 exists to remove: an in-session `nohup … &` launch is not harness-tracked and does not survive reclamation. Relaunched 22:50Z as a harness-tracked background task, which survived 45+ min through the 23:15Z trigger. Lesson pinned in the progress log: heartbeat age > 2h with games in window is FAILED even when artifact age looks fresh; a naive `pgrep -f` guard also matched itself once and skipped a run. Earlier Friday one window (15:16Z) lost.
...
- Daily loop hardened three times this weekend, each a guard reading a store nothing refreshed: L1 projection (57e7a9a, Sat), Statcast pitch store + event->game_pk map (76140d5, Sun). Sunday's loop escalated twice and both were fixed the same morning with regression tests; 09-05 settled (99 wagers) and 09-06 slated (90 wagers). No decision gap on Sunday.
